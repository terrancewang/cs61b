...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("manual (?i)(white|black)$", this::doManual),
        new Command("auto (?i)(white|black)$", this::doAuto),
    ...
    /** Command "new" with Matcher. */
    private void doNew(Matcher unused) {
        _board.init();
        _winner = null;
    }

    /** Command "quit" with Matcher. */
    private void doQuit(Matcher unused) {
        _playing = false;
    }
    /** Command "auto" with MAT.*/
    private void doAuto(Matcher mat) {
        String player = mat.group(1);
        if (player.toUpperCase().equals("WHITE")) {
    ...
    private void doMove(Matcher mat) {
        ...
        if (_board.winner() != null) {
            _reporter.reportNote(String.format("%s%s",
                    _winner.toName(), " wins."));
        }
    }
